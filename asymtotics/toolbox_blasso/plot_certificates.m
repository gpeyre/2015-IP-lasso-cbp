...
plot([0 1], -[1 1], 'k--', 'LineWidth', 2);
axis([0 1 -1.2 1.2]);
set(gca, 'XTick', [], 'YTick', [-1 1], 'FontSize', fs);
set(gca, 'PlotBoxAspectRatio', AR);
box on;
% plot the pre-certificate
h = [];
for i=1:length(Eta)
    h(i) = plot(u, Eta{i}, Col{i}, 'LineWidth', lw);
    if strcmp(Col{i}(1), 'g') % special for green
        set(h(i), 'Color', [0 .4 0]);
    end
end
% add markers
if not(isempty(Mkr))
    ...
    for i=1:length(Eta)
        h = plot(u(t), Eta{i}(t), Col{i}, 'Marker', Mkr{i}, 'MarkerSize', ms, 'LineStyle', 'none');
    end
end
